...
cam = cv2.VideoCapture(2)  # Load the model
model = load_model('model/keras_model.h5')


def image_capture():
    ret, frame = cam.read()
    cv2.imwrite("storage/test.png", frame)


def image_detector(client):
    ...
    image = Image.open('storage/test.png')  # resize the image to a 224x224 with the same strategy as in TM2:
...
def FaceDetector(client):
    image_capture()
    image_detector(client)
